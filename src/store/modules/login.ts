 ...
 * @LastEditTime: 2023-06-27 17:22:52
 */
...
const useLoginStore = defineStore('login', {
  ...
  actions: {
    async loginAccount(account: IAccount) {
      ...
      const id = loginResult.data.userInfo.id
      this.token = loginResult.data.userInfo.token
      ...
      const userMenus = loginResult.data.userMenus
      this.userMenus = userMenus

      // 4.进行本地缓存
      localCache.set('userInfo', loginResult.data.userInfo)
